...
point = Point(-50.17760788437683, -6.058051996789844)

# Approximate degrees for 10 km buffer at the equator (1 degree of lat ~ 111 km at the equator)
buffer_lat_deg = 10 / 111  # Buffer in degrees latitude, which is approximately constant
...
buffer_lon_deg = 10 / (111 * abs(math.cos(math.radians(point.y))))

# Create a box around the point using the approximate buffer
buffered_box = box(point.x - buffer_lon_deg, point.y - buffer_lat_deg,
                   point.x + buffer_lon_deg, point.y + buffer_lat_deg)

# Convert the buffered box to GeoJSON using mapping
buffered_box_geojson = mapping(buffered_box)

search = client.search(
    collections=[collection],
    intersects=buffered_box_geojson,
